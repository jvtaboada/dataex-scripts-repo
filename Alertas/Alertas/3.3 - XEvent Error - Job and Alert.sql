...
/*
Instrução: Verificar se o diretorio abaixo existe no servidor.
	Replace: "D:\Dataex\Traces"
*/
...
IF ( OBJECT_ID('dtx_tb_Log_DB_Error') IS NOT NULL ) 
		DROP TABLE dtx_tb_Log_DB_Error

CREATE TABLE [dbo].[dtx_tb_Log_DB_Error](
...
create nonclustered index Sk01_Log_DB_Error on dtx_tb_Log_DB_Error(err_timestamp,err_message)

GO
IF ( OBJECT_ID('[dbo].dtx_sp_XEvent_DB_Error') IS NOT NULL ) 
	DROP PROCEDURE [dbo].dtx_sp_XEvent_DB_Error
GO
CREATE procedure [dbo].dtx_sp_XEvent_DB_Error
AS
BEGIN
	...
	BEGIN
		...
		from sys.fn_xe_file_target_read_file
			('D:\Dataex\Traces\what_queries_are_failing*.xel',
			'D:\Dataex\Traces\what_queries_are_failing*.xem',
			null, null)
		cross apply (select CAST(event_data as XML) as event_data) as xevents

		insert into dtx_tb_Log_DB_Error([err_timestamp], [err_severity], [err_number], [username], [database_id], [err_message], [sql_text], [client_hostname],[Dt_Error])
		...
		from #events_cte A	
		left join dtx_tb_Log_DB_Error B on A.err_message = B.err_message and A.err_timestamp = B.err_timestamp
	...
	(set filename = 'D:\Dataex\Traces\what_queries_are_failing.xel' ,
	metadatafile = 'D:\Dataex\Traces\what_queries_are_failing.xem',
	max_file_size = 500,
...
IF EXISTS ( SELECT job_id 
            ...
            WHERE name = N'DataEX - Load XEvent Database Error')
	EXEC msdb.dbo.sp_delete_job @job_name = N'DataEX - Load XEvent Database Error', @delete_unused_schedule = 1

GO

/****** Object:  Job [DataEX - XEvent erros Banco de dados]    Script Date: 22/02/2018 19:09:34 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DataEX - Load XEvent Database Error', 
		@enabled=1, 
		...
		@job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DataEX - XEvent error]    Script Date: 22/02/2018 19:09:34 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DataEX - XEvent error', 
		@step_id=1, 
		...
		@command=N'exec dtx_sp_XEvent_DB_Error', 
