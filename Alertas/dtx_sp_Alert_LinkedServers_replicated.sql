...
/****** Object:  StoredProcedure [dbo].[dtx_sp_Alert_LinkedServers_replicated]    Script Date: 9/3/2024 10:43:35 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[dtx_sp_Alert_LinkedServers_replicated]
AS
BEGIN
	SET NOCOUNT ON
	...
	WHERE Nm_Alert = 'LinkedServers replicated'


--Linked Sever

SELECT name
	INTO #TempPrimaryLinkedServers
	FROM sys.servers
	WHERE server_id <> 0

 
SELECT name
	INTO #TempSecondaryLinkedServers
	FROM [LKD_DATAEX].[master].[sys].[servers]
	WHERE server_id <> 0

		IF ( OBJECT_ID('tempdb..#Alert_LinkedServers_replicated') IS NOT NULL ) 
		DROP TABLE #Alert_LinkedServers_replicated

		SELECT AGPRIMARY.name
		INTO #Alert_LinkedServers_replicated
		FROM #TempPrimaryLinkedServers AGPRIMARY
		WHERE NOT EXISTS(SELECT AGSECONDARY.name
		FROM #TempSecondaryLinkedServers AGSECONDARY
		WHERE AGPRIMARY.name = AGSECONDARY.name)

		
IF EXISTS (  SELECT Top 1 name FROM #Alert_LinkedServers_replicated )
BEGIN
...
BEGIN
      		SELECT	name
			INTO ##Email_HTML
			FROM #Alert_LinkedServers_replicated
