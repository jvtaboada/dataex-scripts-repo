...
IF ( OBJECT_ID('[dbo].dtx_tb_Log_AlwaysOn_AG') IS NOT NULL )
	DROP TABLE [dbo].dtx_tb_Log_AlwaysOn_AG;

CREATE TABLE [dbo].[dtx_tb_Log_AlwaysOn_AG](
...
GO 
IF ( OBJECT_ID('[dbo].[dtx_sp_Load_Log_AlwaysOn_AG]') IS NOT NULL ) 
	DROP PROCEDURE [dbo].dtx_sp_Load_Log_AlwaysOn_AG
GO

CREATE procedure [dbo].[dtx_sp_Load_Log_AlwaysOn_AG]
AS

declare @Actual_Date datetime
set @Actual_Date = getdate()

insert into [dtx_tb_Log_AlwaysOn_AG]
SELECT 
	@Actual_Date,
...
GO
IF ( OBJECT_ID('[dbo].[dtx_sp_Alert_AlwaysON_AG_Status]') IS NOT NULL ) 
	DROP PROCEDURE [dbo].dtx_sp_Alert_AlwaysON_AG_Status
GO

CREATE PROCEDURE [dbo].[dtx_sp_Alert_AlwaysON_AG_Status]
AS
BEGIN
	SET NOCOUNT ON
	...
	select @Last_Status = max(Dt_Log) from dtx_tb_Log_AlwaysOn_AG
...
GO
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DataEX - AlwaysOn AG Load and Alert')
EXEC msdb.dbo.sp_delete_job @job_name=N'DataEX - AlwaysOn AG Load and Alert', @delete_unused_schedule=1
GO
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DataEX - AlwaysOn AG Load and Alert', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AlwaysON', 
		...
		@command=N'
exec dtx_sp_Load_Log_AlwaysOn_AG

exec dtx_sp_Alert_AlwaysON_AG_Status
