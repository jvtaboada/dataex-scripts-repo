...
GO
if object_id('dtx_sp_Load_Index_Fragmentation') is not null
	drop procedure dtx_sp_Load_Index_Fragmentation
GO

CREATE procedure [dbo].[dtx_sp_Load_Index_Fragmentation]
AS
BEGIN
	SET NOCOUNT ON
	...
	IF ''?'' IN (''tempdb'',''ReportServerTempDB'',''model'',''master'',''msdb'') 
		RETURN
	ELSE IF EXISTS (SELECT * FROM [dbo].[dtx_tb_Ignore_Databases] WHERE Nm_Database = ''?'')
		RETURN
	...
	INSERT INTO DBADataEX.dbo.dtx_tb_User_Table(Nm_Table)
	SELECT DISTINCT A.Nm_Table 
	FROM ##Index_Fragmentation_History A
		LEFT JOIN DBADataEX.dbo.dtx_tb_User_Table B ON A.Nm_Table = B.Nm_Table
	WHERE B.Nm_Table IS null	
	
    INSERT INTO dbo.dtx_tb_Index_Fragmentation_History(Dt_Log,Id_Server,Id_Database,Id_Table,Nm_Index,Nm_Schema,Avg_Fragmentation_In_Percent,
	...
	JOIN dbo.dtx_tb_User_Server E ON A.Nm_Server = E.Nm_Server 
    LEFT JOIN dtx_tb_Index_Fragmentation_History B ON E.Id_Server = B.Id_Server AND D.Id_Database = B.Id_Database  
...
GO
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DataEX - Load Index Fragmentation')
EXEC msdb.dbo.sp_delete_job @job_name=N'DataEX - Load Index Fragmentation', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DataEX - Load Index Fragmentation', 
		@enabled=1, 
		...
		@owner_login_name=N'saDataEx', 
		@notify_email_operator_name=N'DTX_DBA_Team_Operator',
		@job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DataEX - Carga Fragmentacao Indices]    Script Date: 04/23/2014 20:20:47 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DataEX - Load Index Fragmentation', 
		@step_id=1, 
		...
		@command=N'exec dtx_sp_Load_Index_Fragmentation', 
		@database_name=N'DBADataEX', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DataEX - Index Fragmentation Load', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
