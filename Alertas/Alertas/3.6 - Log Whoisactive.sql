...
if OBJECT_ID('dtx_tb_Log_Whoisactive') is not null
	drop table dtx_tb_Log_Whoisactive

CREATE TABLE dtx_tb_Log_Whoisactive (   
[Dt_Log] datetime,
...
GO
IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DataEX - Load Whoisactive')
EXEC msdb.dbo.sp_delete_job @job_name=N'DataEX - Load Whoisactive', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DataEX - Load Whoisactive', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DataEX - WhoisActive]    Script Date: 04/23/2014 19:59:41 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DataEX - Load Whoisactive', 
		@step_id=1, 
		...
		@command=N'EXEC sp_whoisactive @get_outer_command = 1, @delta_interval = 1,
										...
										@destination_table = ''dtx_tb_Log_Whoisactive''', 
		@database_name=N'DBADataEX', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DataEX - Load Whoisactive', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
