...
-- 1) Fazer um Replace(CTRL + H) no caminho "C:\Dataex\Traces" e alterar para um Caminho Real no servidor. Você precisa dar acesso ao usuário do SQL nessa pasta para criar e excluir arquivo, caso contrário o job de trace vai falhar.


USE DBADataEX -- Database Name

if OBJECT_ID('dtx_tb_Queries_Profile') is not null
	drop table dtx_tb_Queries_Profile
...
if OBJECT_ID('dtx_sp_Trace_Creation') is not null
	drop procedure dtx_sp_Trace_Creation
GO

CREATE  Procedure [dbo].[dtx_sp_Trace_Creation]
AS
BEGIN
	...
	*******************************************************************************************************************************/
	exec @rc = sp_trace_create @TraceID output, 0, N'C:\Dataex\Traces\Duration', @maxfilesize, NULL 
...
if OBJECT_ID('dtx_sp_Load_Queries_Profile') is not null
	drop procedure dtx_sp_Load_Queries_Profile
GO

CREATE PROCEDURE dtx_sp_Load_Queries_Profile
AS
BEGIN
	Insert Into dtx_tb_Queries_Profile(TextData, NTUserName, HostName, ApplicationName, LoginName, SPID, Duration, StartTime, 
	...
	EndTime, Reads,Writes, CPU, ServerName, DataBaseName, RowCounts/*, SessionLoginName*/
	FROM :: fn_trace_gettable('C:\Dataex\Traces\Duration.trc', default)
	...
	IF EXISTS (SELECT * FROM sys.objects WHERE name = 'Alert_Parameter')
		IF EXISTS (SELECT * FROM dtx_tb_Alert_Parameter WHERE Nm_Alert = 'Slow Queries' AND Fl_Enable = 1)
			EXEC dbo.dtx_sp_Alert_Slow_Queries
END
...
IF EXISTS ( SELECT job_id 
            ...
            WHERE name = N'DataEX - Load Server Side Trace')
	EXEC msdb.dbo.sp_delete_job @job_name = N'DataEX - Load Server Side Trace', @delete_unused_schedule = 1
...
BEGIN TRANSACTION
	...
	EXEC	@ReturnCode = msdb.dbo.sp_add_job 
			@job_name = N'DataEX - Load Server Side Trace',
			@enabled = 1,
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			@job_id = @jobId, 
			@step_name = N'DataEX - Disable Trace',
			@step_id = 1,
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0, 
			@subsystem = N'TSQL', 
			@command = N'exec dtx_sp_Load_Queries_Profile', 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0,
			@subsystem=N'CmdExec',
			@command = N'Del "C:\Dataex\Traces\Duration.trc" /Q',
			@flags = 0
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0, 
			@subsystem = N'TSQL', 
			@command = N'exec dbo.dtx_sp_Trace_Creation', 
			@database_name = N'DBADataEX', 
			@flags = 0
...
-- Create Trace
EXEC [dbo].[dtx_sp_Trace_Creation]
