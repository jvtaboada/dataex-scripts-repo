...
--Abrir a procedure dtx_sp_Alert_Every_Day e descomentar a stored procedure call "exec dtx_sp_Alert_DeadLocks"
...
FROM master.sys.fn_xe_telemetry_blob_target_read_file('dl', null, null, null) A
	LEFT JOIN DBADataEX.dbo.Log_DeadLock AS B ON CAST(A.timestamp_utc AS DATETIME) = B.eventDate
...
IF ( OBJECT_ID('[dbo].dtx_sp_XEvent_DeadLock') IS NOT NULL ) 
	DROP PROCEDURE [dbo].dtx_sp_XEvent_DeadLock
GO
...
CREATE procedure [dbo].[dtx_sp_XEvent_DeadLock]
AS
BEGIN
	...
	insert into dtx_tb_Log_DeadLock(eventName,eventDate,deadlock,Nm_Object,Nm_Database)
	...
	CROSS APPLY (
	  ...
	  LEFT JOIN dtx_tb_Log_DeadLock X on dateadd(hh,-3,deadlockData.eventDate) = X.eventDate
	...
	ADD TARGET package0.asynchronous_file_target(
	  SET filename = 'C:\DataEX\Traces\capture_deadlocks.xel',
		  max_file_size = 500,
...
IF ( OBJECT_ID('[dbo].dtx_sp_Alert_DeadLocks') IS NOT NULL ) 
	DROP PROCEDURE [dbo].dtx_sp_Alert_DeadLocks
GO
USE [DBADataEX]
GO
...
CREATE PROCEDURE [dbo].[dtx_sp_Alert_DeadLocks]
AS
BEGIN
	SET NOCOUNT ON
...
IF EXISTS (SELECT job_id 
            ...
            WHERE name = N'DataEX - Load XEvent Deadlock')
EXEC msdb.dbo.sp_delete_job @job_name=N'DataEX - Load XEvent Deadlock'  , @delete_unused_schedule=1

GO
/****** Object:  Job [DataEX - XEvent Deadlock]    Script Date: 25/09/2017 23:40:03 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DataEX - Load XEvent Deadlock', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Executa Procedure', 
		...
		@command=N'exec [dtx_sp_XEvent_DeadLock]', 
...
SELECT * FROM DBADataEX.[dbo].[Log_DeadLock]
...
EXEC msdb.dbo.sp_start_job N'DataEX - XEvent Deadlock'
